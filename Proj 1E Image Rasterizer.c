...
void ImageColor(Image *img, int index, TriangleList *tl, int r, int c, double z4_r, double z4_g, double z4_b)
{
    ...
    unsigned char red = C441(255 * z4_r);
    unsigned char green = C441(255 * z4_g);
    unsigned char blue = C441(255 * z4_b);
...
void RasterizeTriangle(TriangleList* tl, int index, Image *img)
{
   ...
   for (int r = rowMin; r <= rowMax; r++)
   {
	...
	if(intercept1 < intercept2){
     ...
     for(int c = C441(leftEnd); c <= F441(rightEnd); c++)
     {
	     ...
	     {
                  ImageColor(img, index, tl, r, c, z4_r, z4_g, z4_b);
...
double CalculateShading(LightingParameters &idk, double *viewDirection, double *normal)
{
	lp = LightingParameters (maybe init in main)
	double specular = lp->Ks * cos(alpha) ** shininess coefficient;
	double ambient;
	double diffuse;
//calc view direction for each pixel
//call calcshading on each vertex
//lerp this shit
//modify rgb calculation
//do one image right 
...
void TransformAndRenderTriangles(Camera c, TriangleList* tl, Image *img, int i)
{
...
/*	printf("transforming vectors;\n");
	printf("v0 == (%lf, %lf, %lf)\n,", new_v0[0]/new_v0[3], new_v0[1]/new_v0[3], new_v0[2]/new_v0[3]);
	printf("v1 == %lf, %lf, %lf)\n", new_v1[0]/new_v1[3], new_v1[1]/new_v1[3], new_v1[2]/new_v1[3]);
	printf("v2 == %lf, %lf, %lf)\n", new_v2[0]/new_v2[3], new_v2[1]/new_v2[3], new_v2[2]/new_v2[3]);
*/	
...
int main()
{
	...
	if (i % 250 != 0)
     ...
     {
	    TransformAndRenderTriangles(c, tl, img, l);
	    RasterizeTriangle(tl, l, img);
     }
        //char* filename = (char*)malloc(sizeof(char) * 30);
        sprintf(filename, "proj1E_frame%04d.pnm", i);
        //const char* filename = "proj1E_frame0750.pnm";
        //filename = "proj1E_frame" 
